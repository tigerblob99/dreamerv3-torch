...
def _random_crop(img: torch.Tensor, crop_h: int, crop_w: int) -> torch.Tensor:
    """Random crop a single HWC image or NHWC batch."""
    ...
    # Handle batch (B, H, W, C) - Legacy support if needed
    b, h, w, c = img.shape
    if crop_h > h or crop_w > w:
        return img
    top = torch.randint(0, h - crop_h + 1, (b,), device=img.device)
    left = torch.randint(0, w - crop_w + 1, (b,), device=img.device)
    out = torch.empty((b, crop_h, crop_w, c), device=img.device, dtype=img.dtype)
    for i in range(b):
        out[i] = img[i, top[i] : top[i] + crop_h, left[i] : left[i] + crop_w, :]
    return out


def _center_crop(img: torch.Tensor, crop_h: int, crop_w: int) -> torch.Tensor:
    """Center crop a single HWC image or NHWC batch."""
    if crop_h <= 0 or crop_w <= 0:
        return img
...
def BC_MLP_train(config, encoder, action_mlp):
    ...
    # --- NEW: Initialize Datasets and Loaders ---
    ...
    # Eval loader (optional, but good for consistent evaluation)
    ...
    # Keep eval episodes list for visualization (unchanged)
    ...
    # Steps calculation (using loader length)
    ...
    def evaluate():
        """Evaluates the action MLP on evaluation loader."""
        if not eval_loader:
            return None
        action_mlp.eval()
        losses = []
        ...
        # Visualization logic (Reuse existing code logic but adapt if necessary)
        if eval_episodes_list:
            # ... (Visualiztion code can remain mostly same, just ensure it handles the single episode correctly) ...
            # For brevity, reusing the existing visualization block structure is fine 
            # as it samples from eval_episodes_list directly, not the loader.
            pass 
            
            # [Insert visualization block from original file here]

...
python test.py \
  --npz_path datasets/robomimic_data_MV/can_PH_train/demo_181-123.npz \
  --task PickPlaceCan \
  --camera_keys agentview_image robot0_eye_in_hand_image \
  --flip_keys agentview_image robot0_eye_in_hand_image \
  --image_hw 84 84 \
  --out_dir imgs/env_vs_npz
...
def main():
    ...
    env = robosuite.make(
        env_name=args.task,
        robots=args.robots,
        controller_configs=robosuite.load_controller_config(default_controller=args.controller),
        use_object_obs=True,
        use_camera_obs=True,
    ...
    axes[0].imshow(npz_img.astype(np.uint8))
    axes[0].set_title("NPZ image (t=0)")
    axes[1].imshow(env_img.astype(np.uint8))

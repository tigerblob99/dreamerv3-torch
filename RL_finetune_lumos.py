"""LUMOS-style RL fine-tuning over precomputed expert posteriors.

Identical to RL_finetune.py except that the dataset is replaced by
``AC_RL.LumosDataset``, which runs the frozen world model end-to-end on every
expert episode at startup and caches the full posterior trajectory. Each
training batch is then a window of those cached posteriors -- so every
imagination start state has been conditioned on **all** preceding expert
observations within its episode (LUMOS / DITTO warm-up semantics), instead of
the single ``post[:, 0]`` reset that RL_finetune.py's RlDataset path produces.

python RL_finetune_lumos.py --configs robomimic rl_finetune_lumos --env_config can_env_eval
"""
...
from AC_RL.LumosDataset import LumosDataset
...
def _make_batch_iter(dataset, config):
    if dataset.num_episodes == 0:
        raise RuntimeError("LumosDataset is empty.")
    if len(dataset) == 0:
        raise RuntimeError(
            "LumosDataset has no valid windows. Check imag_horizon vs episode lengths."
        )
    cache_device = getattr(dataset, "cache_device", torch.device("cpu"))
    cache_on_gpu = "cuda" in str(cache_device)
    pin_memory = bool(
        torch.cuda.is_available()
        and "cuda" in str(config.device)
        and not cache_on_gpu
    )
...
def rl_finetune(config):
    ...
    # Load raw expert episodes up-front so we can derive observation/action
    # spaces and image-dataset statistics before the WM is built. The actual
    # LumosDataset is constructed later (after the WM is loaded + frozen) so
    # it can pre-encode every episode end-to-end with the frozen WM.
    episodes = tools.load_episodes(
        pathlib.Path(expert_dir).expanduser(),
        limit=getattr(config, "dataset_size", None),
    )
    if len(episodes) == 0:
    ...
    # Now that the WM is loaded + frozen, pre-encode every expert episode end
    # to end so each cached posterior carries full-history context.
    train_dataset = LumosDataset(expert_dir, config, world_model=wm, mode="train")
    ...
    if pretrain_steps > 0:
        ...
        for pre_step in range(pretrain_steps):
            ...
            is_first_horizon = batch["is_first"]
            horizon = expert_action.shape[1]
            ...
            if foc_lambda > 0:
                with tools.RequiresGrad(critic):
                    foc_loss = _compute_foc_loss(
                        wm, critic, reward_model, post, expert_deter,
                        expert_action, horizon, is_first_horizon,
